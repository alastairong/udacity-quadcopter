...
class Hover(BaseTask):
    """Simple task where the goal is to stay at a target height"""

    def __init__(self):
        # State space: <position_x, .._y, .._z, orientation_x, .._y, .._z, .._w>
        cube_size = 300.0  # env is cube_size x cube_size x cube_size
        self.observation_space = spaces.Box(
            np.array([- cube_size / 2, - cube_size / 2,       0.0, -1.0, -1.0, -1.0, -1.0, -8.0, -8.0, -8.0]), # Guessing max velocity is about +/- 8
            np.array([  cube_size / 2,   cube_size / 2, cube_size,  1.0,  1.0,  1.0,  1.0, 8.0, 8.0, 8.0]))
    ...
    def reset(self):
        ...
        p = self.target_position + np.random.normal(0.5, 0.1, size=3) # Starting point randomised around target_z
        return Pose(
                position=Point(*p),
    ...
    def update(self, timestamp, pose, angular_velocity, linear_acceleration):
        ...
        reward = -(self.weight_position * error_position + self.weight_orientation * error_orientation + self.weight_velocity * error_velocity)**2
